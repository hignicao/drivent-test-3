...
describe('GET /hotels', () => {
  ...
  describe('when token is valid', () => {
    ...
    it('should respond with status 402 when ticket has no hotel', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
    ...
    it('should respond with status 402 when ticket isnt paid yed', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
    ...
    it('should respond with status 200 when ticket is paid and has hotel', async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createTicketTypeWithHotel();
